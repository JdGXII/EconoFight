...
{
	public partial class MundoHayek
	{
        bool pausa;
        double gameover;
        bool murio;
  ...
  FlatRedBallServices.Load<Song>(@"Content/cancion 1", ContentManagerName);
		...
		{
            ListaJugadores[0].ActividadNormal(GlobalData.getControl1(), HayekInstance.getPosicion());
            HayekInstance.ActividadAI(ListaJugadores[0].getPosicion());
        ...
        {
            HayekInstance.setPosicion(230, -120);
            HayekInstance.getSensorial().setPosicion(240, 200);
            HayekInstance.getHP().SetPosicionFlip(-210, 240);
            HayekInstance.VoltearPersonaje();
            Entities.SuperBarra barra = new Entities.SuperBarra(ContentManagerName);
            barra.SetPosicionFlip(HayekInstance.X, -250 - 20);
            HayekInstance.LlenarBarra(barra);
           // ListaJugadores.Add(HayekInstance);
        }

        private void ColisionMundo()
        {
            ListaJugadores[0].getCuerpo().CollideAgainstMove(this.LimitesMundo, 0, 1);
            HayekInstance.getCuerpo().CollideAgainstMove(this.LimitesMundo, 0, 1);
            
        }

        private void ColisionEntreJugadores()
        {
            ListaJugadores[0].getCuerpo().CollideAgainstMove(HayekInstance.getCuerpo(), 1, 1);
            HayekInstance.getCuerpo().CollideAgainstMove(ListaJugadores[0].getCuerpo(), 1, 1);
        }

        private void ColisionPhys()
        {
            
             if (HayekInstance.getCuerpo().CollideAgainst(ListaJugadores[0].getGolpe()))
            {
               HayekInstance.RecibirDañoFlip(ListaJugadores[0].DarDañoFisico());
            }
        ...
        private void ColisionAtaqueJugador1()
        {
            if (ListaJugadores[0].ListaAtaques.Count > 0)
            {
                ...
                {
                    if (HayekInstance.getCuerpo().CollideAgainst(i.getCuerpo()))
                    {

                        HayekInstance.RecibirDañoFlip(i.giveDamage());
        ...
        {
            if (HayekInstance.ListaAtaques.Count > 0)
            {
                foreach (Entities.Especial i in HayekInstance.ListaAtaques)
                {
                    if (ListaJugadores[0].getCuerpo().CollideAgainst(i.getCuerpo()))
                    {

                        ListaJugadores[0].RecibirDaño(i.giveDamage());
                        HayekInstance.ActualizarBarraFlip();
                        i.Destroy();
            ...
            if (HayekInstance.ListaAtaqueSuper.Count > 0)
            {
                foreach (Entities.SuperAtaque i in HayekInstance.ListaAtaqueSuper)
                {
            ...
            if (ListaJugadores[0].ListaAtaques.Count > 0)
            {
                if (HayekInstance.ListaAtaques.Count > 0)
                {
                    foreach (Entities.Especial i in ListaJugadores[0].ListaAtaques)
                    {
                        ...
                        foreach (Entities.Especial j in HayekInstance.ListaAtaques)
                        {
            ...
            if (ListaJugadores[0].ListaAtaqueSuper.Count > 0)
            {
                if (HayekInstance.ListaAtaqueSuper.Count > 0)
                {
                    foreach (Entities.SuperAtaque i in ListaJugadores[0].ListaAtaqueSuper)
                    {
                        ...
                        foreach (Entities.SuperAtaque j in HayekInstance.ListaAtaqueSuper)
                        {
        ...
        private void ColisionBloqueo1()
        {
            if (ListaJugadores[0].ListaAtaques.Count > 0)
            {
                foreach (Entities.Especial i in ListaJugadores[0].ListaAtaques)
                {
                    if (i.getCuerpo().CollideAgainst(HayekInstance.getShield()))
                    {
                        i.Destroy();
                    }
                }
        ...
        private void FightOver()
        {
            ...
            else if (HayekInstance.getAlive() == false && murio == false)
            {
                gameover = TimeManager.CurrentTime;
            ...
            else if (HayekInstance.getAlive() == false && ListaJugadores[0].getAlive() == false && murio == false)
            {
                gameover = TimeManager.CurrentTime;
        ...
        private void NuevaPantalla()
        {
            ...
            if (TimeManager.CurrentTime - gameover >= 3.0)
            {
                ...
                {
                    MoveToScreen(typeof(TutorialKeynes).FullName);
                }
